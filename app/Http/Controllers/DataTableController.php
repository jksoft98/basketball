...
class DataTableController extends Controller
{
    ...
    public function sessions(Request $request)
    {
        ...
        $query = TrainingSession::whereIn('batch_id', $batchIds)
            ->with(['batch', 'batch.coaches', 'attendances']);    // ← Change 2a
        ...
        $data = $sessions->map(function ($session) use ($typeColors) {
            ...
            // ← Change 2b — coaches instead of coach
            $batchHtml = '<div class="font-medium text-gray-900">'.$session->batch->name.'</div>'.
                         '<div class="text-xs text-gray-400">'.$session->batch->coaches->pluck('name')->join(', ').'</div>';
    ...
    {
        $user  = auth()->user();
        $query = $user->isAdmin()                                 // ← Change 3a
            ? Batch::with('coaches')->withCount('students')
            : $user->batches()->with('coaches')->withCount('students');
        ...
        $data = $batches->map(function ($batch) {
            ...
            return [
                'name'     => $nameHtml,
                'coach'    => e($batch->coaches->pluck('name')->join(', ') ?: '—'), // ← Change 3b
                'level'    => $levelHtml,
    ...
    {
        $user     = auth()->user();
        $batchIds = $user->isAdmin()                              // ← Change 4a
            ? Batch::pluck('id')
            : $user->batches()->pluck('batches.id');
        ...
        $data = $sessions->map(function ($session) use ($typeColors) {
            // ← Change 4b — joined_at + deleted_at filter
            $total = Student::withTrashed()
                ->where('batch_id', $session->batch_id)
                ->where(function ($q) use ($session) {
                    $q->whereNull('joined_at')
    ...
    public function sessionAttendance(Request $request, TrainingSession $session)
    {
        ...
        $data = $students->map(function ($student) use ($attendanceMap, $noteMap) {
            ...
            $photoHtml = '<div class="flex items-center gap-3">'.
                '<div class="relative flex-shrink-0">'.
                    '<img src="'.e(asset('storage/' . $student->photo_thumb_path)).'" alt="'.e($student->full_name).'" '.
                        'class="w-9 h-9 rounded-full object-cover bg-gray-100" loading="lazy">'.
                    ($student->isAtRisk()
