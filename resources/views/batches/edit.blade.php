    ...
    <form action="{{ route('batches.update', $batch) }}" method="POST" class="space-y-5">
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-6 space-y-4">
            <div>
                ...
                <input type="text" name="name" value="{{ old('name', $batch->name) }}" class="w-full h-10 px-3 text-sm border border-gray-200 rounded-lg focus:outline-none focus:border-orange-400 bg-white" placeholder="e.g. Under-16 Squad A" required>
                @error('name')<p class="text-red-500 text-xs mt-1">{{ $message }}</p>@enderror
            </div>
            <div>
                <label class="block text-xs font-medium text-gray-600 mb-1.5">Description</label>
                <input type="text" name="description" value="{{ old('description', $batch->description) }}" class="w-full h-10 px-3 text-sm border border-gray-200 rounded-lg focus:outline-none focus:border-orange-400 bg-white" placeholder="Optional short description">
            </div>
            <div>
                ...
                <select name="coach_id" class="w-full h-10 px-3 text-sm border border-gray-200 rounded-lg focus:outline-none focus:border-orange-400 bg-white" required>
                    ...
                    <option value="{{ $coach->id }}" {{ old('coach_id', $batch->coach_id)==$coach->id ? 'selected' : '' }}>{{ $coach->name }}</option>
                    @endforeach
                </select>
